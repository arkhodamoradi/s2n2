...
class SpikingResNet(nn.Module):
    def __init__(self, block, input_size, synapse_reset, threshs, nb_classes=11):
        ...
        self.custom_pool1 = nn.Conv2d(self.out1plane, 8, kernel_size=5, bias=False)
        self.synapse1 = Synapse(threshold=threshs[1], reset=self.synapse_reset)

        self.fc_custom1 = nn.Linear(1152, 128, bias=False)
        self.synapse2 = Synapse(threshold=threshs[2], reset=self.synapse_reset)

        self.fc_custom2 = nn.Linear(128, nb_classes, bias=False)
    ...
    def forward(self, _input, time_window, reset):
        if reset:
            ...
            self.c1_spk = self.c1_mem = torch.zeros(self.batch_size, 8, 12, 12, device=device)
            self.fc1_spk = self.fc1_mem = torch.zeros(self.batch_size, 128, device=device)
        ...
        for step in range(time_window):
            ...
            x = self.fc_custom1(x)
            x, self.fc1_mem, self.fc1_spk = self.synapse2.mem_update(x, self.fc1_mem, self.fc1_spk)
            r[2] += x.nonzero().size(0) / x.numel()

            x = self.fc_custom2(x)
            x, self.fc2_mem, self.fc2_spk = self.synapse3.mem_update(x, self.fc2_mem, self.fc2_spk)
...
def spiking_resnet_18(input_size, synapse_reset, threshs, nb_classes=11):
